...
class MemoriesViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        SocketIOManager.sharedInstance.receivedAllMemories(completionHandler: { (newMemories) -> Void in
            DispatchQueue.main.async {
                ...
                for newMemory in newMemories ?? [["System", "No new messages!"]] {
                    let newDateType = dateFormatter.date(from: newMemory[1])! as NSDate
                    let memoryToAdd = memory(title: newMemory[0], date: newMemory[1], dateType: newDateType, description: newMemory[2])
        ...
        SocketIOManager.sharedInstance.getMemory(completionHandler: { (newTitle, newDate, newDescription) -> Void in
            DispatchQueue.main.async {
                let dateFormatter = DateFormatter()
                dateFormatter.locale = Locale(identifier: "en_US_POSIX")
                dateFormatter.timeZone = TimeZone.autoupdatingCurrent
                ...
                let memoryToAdd = memory(title: newTitle, date: newDate, dateType: newDateType, description: newDescription)
                self.memories.append(memoryToAdd)
                self.memoriesTableView.reloadData()
            }
        })
    ...
    @IBAction func createNewMemory(_ sender: Any) {
        ...
        let memoryToAdd = memory(title: newMemoryTitle.text!, date: newMemoryDate.text!, dateType: newDateType, description: newMemoryDescription.text!)
        
        SocketIOManager.sharedInstance.sendMemory(sender: defaults.string(forKey: "localUsername") ?? "Default", title: memoryToAdd.title, date: memoryToAdd.date, description: memoryToAdd.description)
